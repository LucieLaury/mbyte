 ...
 */
package fr.jayblanc.mbyte.manager.runtime.dokku;

import com.jcraft.jsch.*;
import fr.jayblanc.mbyte.manager.runtime.RuntimeProvider;
import fr.jayblanc.mbyte.manager.runtime.RuntimeProviderConfig;
...
@Singleton
public class DokkuRuntimeProvider implements RuntimeProvider {

    private static final Logger LOGGER = Logger.getLogger(DokkuRuntimeProvider.class.getName());
    private static final String NAME = "dokku";

    @Inject RuntimeProviderConfig storeConfig;
    ...
    @Override
    public List<String> listAllStores() throws RuntimeProviderException {
        ...
        try {
            ...
            if ( status != 0 ) {
                throw new RuntimeProviderException("unable to list apps: " + output);
            }
            return Arrays.stream(output.toString().split("\\r?\\n")).filter(s -> !s.startsWith("=====>")).collect(Collectors.toList());
        } catch ( IOException | JSchException e ) {
            throw new RuntimeProviderException("unable to list apps", e);
        }
    }
    ...
    * */
    ...
    @Override
    public String startStore(String id, String name, String owner) throws RuntimeProviderException {
        ...
        cmds.add("config:set " + id + " WILDFLY_ADMIN_PASSWORD=filestore" +
                ...
                " FILESTORE_NAME=" + name +
                " FILESTORE_ID=" + id +
                " FILESTORE_FQDN=" + id + "." + config.host());
        cmds.add("ports:add " + id + " http:80:8080");
        cmds.add("git:from-image " + id + " " + config.image());
        ...
        try {
            ...
            for (String cmd : cmds) {
                ...
                if (status != 0) {
                    throw new RuntimeProviderException("error during app creation: " + output);
                }
            }
            return output.toString();
        } catch ( IOException | JSchException e ) {
            throw new RuntimeProviderException("Unable to create app", e);
        }
    }

    @Override
    public String stopStore(String id) throws RuntimeProviderException {
        throw new RuntimeProviderException("Not implemented");
    }
    ...
    public String destroyStore(String id) throws RuntimeProviderException {
        ...
        try {
            ...
            if ( status != 0 ) {
                throw new RuntimeProviderException("unable to delete app: " + output);
            }
            LOGGER.log(Level.INFO, "[output] {}", output.toString());
        } catch ( IOException | JSchException e ) {
            throw new RuntimeProviderException("Unable to delete app", e);
        }
