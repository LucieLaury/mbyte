 ...
 */
@QuarkusTest
public class CreateVolumeTaskTest {

    private static final Logger LOGGER = Logger.getLogger(CreateVolumeTaskTest.class.getName());
    private static final String VOLUME_NAME_PREFIX = "test_create_volume_";

    @Inject CreateVolumeTaskHandler handler;
    ...
    public void testCreateExistingVolume() throws TaskException {
        ...
        context.setValue(CreateVolumeTaskHandler.VOLUME_NAME, volumeName);
        context.setValue(CreateVolumeTaskHandler.FAIL_IF_EXISTS, "false");
    ...
    public void testCreateVolumeFailIfExists() {
        ...
        ProcessContext context = new ProcessContext();
        context.setValue(CreateVolumeTaskHandler.VOLUME_NAME, volumeName);
        context.setValue(CreateVolumeTaskHandler.FAIL_IF_EXISTS, "true");
        handler.setContext(context);
    ...
    public void testCreateVolume() throws TaskException {
        ...
        assertTrue(handler.getLog().contains("Volume create with name: '" + volumeName + "'"), "Expected log to contain success message for created volume");
        assertEquals(TaskStatus.COMPLETED, handler.getStatus(), "Expected task status to be COMPLETED for created volume");

        boolean created = client.listVolumesCmd().exec().getVolumes().stream()
