 ...
 */
@QuarkusTest
public class CreateDockerNetworkTaskTest {

    private static final Logger LOGGER = Logger.getLogger(CreateDockerNetworkTaskTest.class.getName());
    private static final String NETWORK_NAME_PREFIX = "test_create_docker_network_";

    @Inject CreateDockerNetworkTask handler;
    ...
    public void testCreateNetwork() throws TaskException {
        ...
        handler.setContext(context);
        handler.execute();
        assertTrue(handler.getLog().contains("Network created with name: '" + networkName + "'"), "Expected log to contain success message for created network");
